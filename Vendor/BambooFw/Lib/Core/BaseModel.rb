	...
	class Lib < BaseClass
	    ...
	    class BaseModel < BaseClass
	      def initialize(config)
		        ...
		        @logger.log('debug', ['BambooFw::Lib::Core::BaseModel', 'initialize', "table_name:#{table_name}"])
		        ...
		        @logger.log('debug', ['BambooFw::Lib::Core::BaseModel', 'initialize', "Error:#{e.message}"])
					end
	      end

		    def find_all
					@logger.log('debug', ['BambooFw::Lib::Core::BaseModel', 'find_all', "find_all START"])
					begin
			    	data = []
			    	sql = "SELECT * FROM #{@table_name} AS #{@table_name}"
		        @logger.log('debug', ['BambooFw::Lib::Core::BaseModel', 'find_all', "sql:#{sql}"])
			    	rs = @connection.query(sql)
						...
						@logger.log('debug', ['BambooFw::Lib::Core::BaseModel', 'find_all', "Error:#{e.message}"])
					end
		    end

		    def find(id)
	        @logger.log('debug', ['BambooFw::Lib::Core::BaseModel', 'find', 'START'])
	        data = nil
		    	begin
		        @logger.log('debug', ['BambooFw::Lib::Core::BaseModel', 'find', "id:" + id.inspect])
		        ...
		        @logger.log('debug', ['BambooFw::Lib::Core::BaseModel', 'find', "Error:#{e.message}"])
		    	end
		    end
