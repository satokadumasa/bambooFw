	...
	class Lib < BaseClass
		class Core < BaseClass
			class Router < BaseClass
		        ...
		        @logger.log('debug', ['BambooFw::Lib::Core::Router', 'initialize', "default_actions:#{@default_actions.inspect}"])
					rescue Exception => e
						# puts "BambooFw::Lib::Core::Router.initialize Error:#{e.massage}"
					end
				end
					...
					begin
						...
						@yaml.each do |key, value|
					  	arr_route = []
							controller_name = ''
							action = ''
			        @logger.log('debug', ['BambooFw::Lib::Core::Router', 'generate_routes', "CH-00"])
			        if value.include?('#')
			        	arr = value.split('#')
			        	controller = arr[0]
			        	action = arr[1]
		        ...
		        # @logger.log('debug', ['BambooFw::Lib::Core::Router', 'generate_routes', "routes:#{@routes}"])
					rescue Exception => e
		        @logger.log('debug', ['BambooFw::Lib::Core::Router', 'generate_routes', "Error:#{e.message}"])
					end
				end
				...
				def find_route(uri, method_type)
			       ...
			       @logger.log('debug', ['BambooFw::Lib::Core::Router', 'find_route', e.message])
					end
				end
			end
		end
